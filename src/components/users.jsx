...
function Users() {
    ...
    if (!users.length) {
        return <h1><span className="badge bg-danger p-2">{renderPhrase(users.length)}</span></h1>
    }

  return (<>
    <h1><span className="badge bg-primary p-2">{renderPhrase(users.length)}</span></h1>
    <table className="table">
        ...
        {users.map(user => {
            return (
                <tr key={user._id}>
                    <td>{user.name}</td>
                    <td>{user.qualities.map((quality,index) => <span key={index} className={`badge rounded-pill bg-${quality.color}`}>{quality.name}</span>)}</td>
